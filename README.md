...
所有订阅格式均支持本地转换，节点信息（服务器地址、密码、UUID 等）不会发送给任何第三方。

**使用方式：**

1. 进入 `设置` → `外部服务集成` 卡片
2. 开启 `内置转换器（隐私模式）` 开关（默认已开启）
3. 选择分流规则模板：

| 模板 | 说明 | 适用场景 |
|------|------|---------|
| 基础 | 仅 GEOIP 分流（CN 直连 + 其余代理） | 追求极简、不需要细分规则 |
| 标准（推荐） | Apple 直连 + 全球媒体/Telegram 代理 + 国内直连 | 日常使用，平衡隐私与便利 |
| 完整 | YouTube/Netflix/Disney+/Spotify/Telegram/Microsoft/Game 细分策略组 | 需要精细控制每个服务的出口节点 |

**隐私说明：**
- 开启时：所有格式转换在本地完成，零数据外泄
- 规则集从公开仓库加载（[blackmatrix7/ios_rule_script](https://github.com/blackmatrix7/ios_rule_script)），仅包含域名/IP 分类数据，不涉及任何隐私信息
- 关闭时：将使用外部 SubConverter 后端，节点信息会明文发送给该后端

**注意事项：**
- Surfboard 不支持远程规则集，始终使用基础内联规则（GEOIP + FINAL）
- 即使外部规则仓库不可用，内置基础规则仍可正常工作
...
**✨ 内置转换器全格式覆盖 + 规则模板系统:**
- **全格式内置转换** - 新增 Sing-Box、Quantumult X、Surfboard 内置生成器，所有格式均可本地转换，彻底消除第三方隐私风险
- **规则模板系统** - 支持基础/标准/完整三档分流预设，规则集从公开仓库加载，零隐私风险
- **隐私模式开关** - 设置页新增内置转换器开关，一键切换本地/外部转换模式
- **规则模板选择器** - 内置模式下可选择分流规则精细度，完整模板对齐 ACL4SSR_Online_Full 级别

**🔧 技术改进:**
- 6 个内置生成器（Clash/Surge/Loon/Sing-Box/QX/Surfboard）均接入规则模板系统
- Surfboard 因格式限制始终使用基础内联规则
- 默认开启内置转换器，默认标准规则模板
