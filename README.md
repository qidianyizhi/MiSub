...
4. （可选）为单个订阅组指定独立规则模板：编辑 Profile → 高级设置 → 规则模板，留空则使用全局设置

**隐私说明：**
- 开启时：所有格式转换在本地完成，零数据外泄；外部后端地址/配置文件/测试按钮自动隐藏
- 关闭时：将使用外部 SubConverter 后端，节点信息会明文发送给该后端；可在 Profile 高级设置中自定义后端和远程配置
